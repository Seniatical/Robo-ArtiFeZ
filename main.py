...
for file in os.listdir('./cogs'):
    if file.endswith('.py'):
        try:
            bot.load_extension(f'cogs.{file[:-3]}'), loaded += 1
        except Exception as error:
            traceback.print_exception(etype=type(error), value=error, tb=error.__traceback__), not_loaded += 1
...
@bot.event
async def on_guild_join(guild) -> discord.Message or None:
...
async def ping(ctx: commands.Context) -> discord.Embed:
